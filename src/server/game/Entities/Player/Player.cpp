...
bool Player::CheckInstanceLoginValid()
{
	...
	//判断副本人数，不能大于限定。
		if (GetMap()->IsDungeon())
		{
			TC_LOG_ERROR(LOG_FILTER_PLAYER, "m_raidDifficulty= %d , GetRaidDifficulty()=%d.", m_raidDifficulty,GetRaidDifficulty());
			TC_LOG_ERROR(LOG_FILTER_PLAYER, "GetMap()->GetPlayersCountExceptGMs()= %d .", GetMap()->GetPlayersCountExceptGMs());
				if(GetMap()->GetPlayersCountExceptGMs() > 2 &&( m_raidDifficulty == 0 || m_raidDifficulty == 2 ) )
				{
//					TeleportTo(m_homebindMapId, m_homebindX, m_homebindY, m_homebindZ, GetOrientation());
					TC_LOG_ERROR(LOG_FILTER_PLAYER, "ERROR!!PlayersCount = %d .", GetMap()->GetPlayersCountExceptGMs());
					return false;
				}
				if(GetMap()->GetPlayersCountExceptGMs() > 3 && (m_raidDifficulty == 1 || m_raidDifficulty == 3 ) )
				{
//					TeleportTo(m_homebindMapId, m_homebindX, m_homebindY, m_homebindZ, GetOrientation());
					TC_LOG_ERROR(LOG_FILTER_PLAYER, "ERROR!!PlayersCount = %d .", GetMap()->GetPlayersCountExceptGMs());
					return false;
